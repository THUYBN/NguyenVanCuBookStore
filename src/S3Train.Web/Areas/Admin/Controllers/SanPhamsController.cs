...
namespace S3Train.Web.Areas.Admin.Controllers
{
    public class SanPhamsController : Controller
    {
        ...
        public ActionResult Index(int? page, string search)
        {
            ...
            if (search == null)
            {
                var sanPhams = db.SanPhams.Include(s => s.LoaiSanPham).Include(s => s.NgonNgu).Include(s => s.NhaCungCap).Include(s => s.NhaSanXuat).Where(s => s.LoaiSanPham.PhanLoai.TenPhanLoai == "Sách");
                return View(sanPhams.ToList().OrderBy(t => t.NgayCapNhat).ToPagedList(pageNumber, pageSize));
            }
            else
            {
                var sanPhams = db.SanPhams.Include(s => s.LoaiSanPham).Include(s => s.NgonNgu).Include(s => s.NhaCungCap).Include(s => s.NhaSanXuat).Where(s => s.LoaiSanPham.PhanLoai.TenPhanLoai == "Sách" && (s.TenSP.Contains(search) || s.TacGia.Contains(search)));
                return View(sanPhams.ToList().OrderBy(t => t.NgayCapNhat).ToPagedList(pageNumber, pageSize));
        ...
        public ActionResult Create([Bind(Include = "Id,TenSP,TacGia,Gia,HinhAnh,TrongLuong,SoLuong,SoTrang,KichThuoc,GhiChu,NgayTao,NgayCapNhat,ChatLuong,MauSac,TrangThai,MaNN,MaNSX,MaNCC,MaLoai,LuotXem")] SanPham sanPham, HttpPostedFileBase fileUpload)
        {
